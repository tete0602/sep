...
class QNetwork(tf.keras.Model):
    def __init__(self):
        ...
        # 各变量具体尺寸如下：
        # 对于尺寸为[m, n]的二维张量input， tf.layers.dense()会生成：尺寸为[n, k]的权矩阵kernel，和尺寸为[m, k]的偏移项bias。
    ...
    for episode_id in range(num_episodes):
        ...
        for t in range(max_len_episode):
            ...
            if len(replay_buffer) >= batch_size:
                ...
                batch_state, batch_reward, batch_next_state, batch_done = [ np.array(a, dtype=np.float32) \
                                                                            for a in [batch_state, batch_reward,\
                                                                                      batch_next_state, batch_done]]
                batch_action = np.array(batch_action, dtype=np.int32)

                q_value = model(batch_next_state)
                # reduce_max axis=1每一行的最大值，=0 每一列的最大值
